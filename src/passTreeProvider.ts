...
type PassCategory = 'GIMPLE Passes'|'IPA Passes'|'RTL Passes'|'DOT Files'|
    'GIMPLE Graphs'|'IPA Graphs'|'RTL Graphs';
...
export class GccPassTreeProvider implements vscode.TreeDataProvider<PassItem> {
  private _onDidChangeTreeData:
      vscode.EventEmitter<PassItem|undefined|null|void> =
      new vscode.EventEmitter<PassItem|undefined|null|void>();
  readonly onDidChangeTreeData: vscode.Event<PassItem|undefined|null|void> =
  ...
  getTreeItem(element: PassItem): vscode.TreeItem { return element; }
  ...
  private async getTestcaseRoots(): Promise<PassItem[]> {
    ...
    try {
      ...
      return Array.from(baseNames).sort().map(name =>
                                                  new PassItem(name, 'root'));
    } catch (e) {
      return [];
    }
  }

  private async getPassFiles(baseName: string, category: string,
                             isDot: boolean): Promise<PassItem[]> {
    if (!this.currentDir)
      return [];
    const files = await fs.promises.readdir(this.currentDir);
    ...
    for (const f of files) {
      ...
      if (match) {
        ...
        if (fBase === baseName && type === targetChar) {
          ...
          passFiles.push(
              new PassItem(isDot ? label : passName, 'file',
                           vscode.Uri.file(path.join(this.currentDir, f)),
                           baseName, parseInt(numStr, 10), `Pass ${numStr}`));
        }
      }
    }
