...
export class RtlDefCache {
    ...
    private parseRegNotes(content: string) {
        ...
        const isDef = (line: string) => /^(REG_NOTE|REG_CFA_NOTE)\s*\(/.test(line.trim());
    ...
    private extractAndStoreDoc(name: string, currentIndex: number, lines: string[], isDefFn: (l: string) => boolean) {
        ...
        for (let j = currentIndex - 1; j >= 0; j--) {
            ...
            // 1. Stop conditions
            if (prevLine === '') break; // Empty line = End of doc block
            if (prevLine.startsWith('#')) continue; // Skip preprocessor (#define, #ifdef)
