...
class GccMdSymbolProvider {
    async provideDefinition(document, position) {
        ...
        for (const filePath of sortedFiles) {
            ...
            const pattern = new RegExp(`\\(define_(attr\\s+"${word}"|[a-z]+_(iterator|attr)\\s+${word}\\b)|\\(\\s*${word}\\s+([0-9a-fAxX]+|[-0-9]+)\\s*\\)`, 'm');
            const match = content.match(pattern);
            if (match && match.index !== undefined) {
                const textBefore = content.substring(0, match.index);
                const lines = textBefore.split('\n');
                return new vscode.Location(vscode.Uri.file(filePath), new vscode.Position(lines.length - 1, lines[lines.length - 1].length));
            }
        }
        return null;
    }
