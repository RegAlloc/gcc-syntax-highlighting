...
exports.GccPipelineProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
class GccPipelineProvider {
    static viewType = 'gcc.pipelineView';
    async show(editor, extensionUri) {
        ...
        if (dumps.length === 0) {
            const item = await vscode.window.showErrorMessage(`No GCC dumps found for '${sourceFileName}'.`, "How to Fix?", "Close");
            if (item === "How to Fix?") {
                vscode.window.showInformationMessage(`Compile with:  gcc -fdump-tree-all -fdump-rtl-all ${sourceFileName}`);
            }
            return;
        }
        const panel = vscode.window.createWebviewPanel(GccPipelineProvider.viewType, `Pipeline: ${sourceFileName}`, vscode.ViewColumn.Two, { enableScripts: true });
    ...
    async analyzeDumps(dir, sourceName) {
        let files = [];
        try {
    ...
    // --- THE FILTER LOGIC ---
    async getCleanLines(filePath, type) {
        try {
...
exports.GccPipelineProvider = GccPipelineProvider;
function escapeRegExp(string) {
    return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
//# sourceMappingURL=pipelineProvider.js.map